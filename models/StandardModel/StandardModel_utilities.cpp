...
StandardModel_spectrum_plotter::StandardModel_spectrum_plotter()
   : spectrum()
   , scale(0.0)
   , width(16)
{
...
void StandardModel_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name.c_str(), std::ios::out);
   VERBOSE_MSG("StandardModel_spectrum_plotter::write_to_file: opening file: "
               << file_name.c_str());
   if (filestr.fail()) {
      ERROR("StandardModel_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("StandardModel_spectrum_plotter::write_to_file: file written: "
               << file_name.c_str());
}

void StandardModel_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   for (std::size_t s = 0; s < spectrum.size(); ++s) {
      if (!filestr.good()) {
         ERROR("StandardModel_spectrum_plotter::write_spectrum: "
...
std::valarray<double> StandardModel_spectrum_plotter::to_valarray(double v)
{
