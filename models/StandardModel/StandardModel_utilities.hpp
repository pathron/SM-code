...
#ifndef StandardModel_UTILITIES_H
#define StandardModel_UTILITIES_H
...
class StandardModel_parameter_getter {
public:
   Eigen::ArrayXd get_parameters(const StandardModel<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const StandardModel<Two_scale>&) const {
...
class StandardModel_spectrum_plotter {
public:
   StandardModel_spectrum_plotter();
   ~StandardModel_spectrum_plotter() {}

   template <class T>
   void extract_spectrum(const StandardModel<T>&);
...
template <class T>
void StandardModel_spectrum_plotter::extract_spectrum(const StandardModel<T>& model)
{
   spectrum.clear();
...
template <class Scalar, int M, int N>
std::valarray<double> StandardModel_spectrum_plotter::to_valarray(const Eigen::Array<Scalar, M, N>& v)
{
