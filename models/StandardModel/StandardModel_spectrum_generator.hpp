...
// File generated at Sat 29 Nov 2014 17:55:14

#ifndef StandardModel_SPECTRUM_GENERATOR_H
#define StandardModel_SPECTRUM_GENERATOR_H
...
#include "StandardModel_two_scale_initial_guesser.hpp"
#include "StandardModel_utilities.hpp"
...
template <class T>
class StandardModel_spectrum_generator {
public:
   StandardModel_spectrum_generator()
      : solver(), model()
   ...
   const StandardModel<T>& get_model() const { return model; }
   ...
   void run(const QedQcd& oneset, const StandardModel_input_parameters& input);
   void write_running_couplings(const std::string& filename = "StandardModel_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "StandardModel_spectrum.dat") const;

private:
   RGFlow<T> solver;
   StandardModel<T> model;
   StandardModel_susy_scale_constraint<T> susy_scale_constraint;
   StandardModel_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void StandardModel_spectrum_generator<T>::run(const QedQcd& oneset,
                                const StandardModel_input_parameters& input)
{
   ...
   StandardModel_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   StandardModel_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint);
 ...
 */
template <class T>
void StandardModel_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   StandardModel<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   StandardModel_parameter_getter parameter_getter;
   Coupling_monitor<StandardModel<T>, StandardModel_parameter_getter>
 ...
 */
template <class T>
void StandardModel_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   StandardModel_spectrum_plotter plotter;
